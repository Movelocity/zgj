...
type InvitationUse struct {
	...
	InvitationCode string    `json:"invitation_code" gorm:"type:varchar(32);index:idx_invitation_code;uniqueIndex:idx_user_invitation;not null;comment:使用的邀请码"`
	UsedBy         string    `json:"used_by" gorm:"type:varchar(20);index:idx_used_by;uniqueIndex:idx_user_invitation;not null;comment:使用者用户ID"`
