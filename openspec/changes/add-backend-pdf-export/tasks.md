...
## 1. 数据库层 ✅ 核心 (MVP简化版)
- [ ] 1.1 创建 `server/model/pdf_export_task.go` 模型
  - [ ] 1.1.1 定义 `PdfExportTask` 结构体（MVP字段）
    ```go
    ...
    ```
  - [ ] 1.1.2 定义 TableName() 方法返回 "pdf_export_tasks"
  - [ ] 1.1.3 定义状态常量（StatusPending, StatusProcessing, StatusCompleted, StatusFailed）
- [ ] 1.2 在 `server/initialize/db.go` 添加自动迁移
- [ ] 1.3 在 `server/model/enter.go` 导出新模型
...
## 2. Go后端配置层 ✅ 核心 (MVP简化版)
- [ ] 2.1 在 `server/config/config.go` 添加 PdfExport 配置结构
  ```go
  type PdfExportConfig struct {
      NodeServiceURL string `mapstructure:"node_service_url"` // Node.js服务地址
  }
  ```
- [ ] 2.2 在 `server/config.example.yaml` 和 `server/config.yaml` 添加配置
  ```yaml
  pdf_export:
...
## 3. Go后端服务层 ✅ 核心 (MVP简化版)
- [ ] 3.1 创建 `server/service/pdfexport/pdf_export_service.go`
  - [ ] 3.1.1 实现 CreateExportTask(userID, resumeID string) (string, error) 函数
    ```go
    // 1. 查询简历记录
    // 2. 生成任务ID（TLID）
    ...
    // 5. 返回任务ID
    ```
  - [ ] 3.1.2 实现 GetTaskStatus(taskID string) (*model.PdfExportTask, error) 函数
    ```go
    // 1. 查询任务记录
    // 2. 返回任务状态（不验证权限，MVP简化）
    ```
  - [ ] 3.1.3 实现 GeneratePdfAsync(taskID string, resumeData map[string]interface{}) 函数
    ```go
    // 1. 更新任务状态为processing
    ...
    ```
  - [ ] 3.1.4 实现 SavePdfFile(taskID string, fileData []byte) error 函数
    ```go
    // 1. 创建目录 server/uploads/pdf/YYYY-MM-DD/
    // 2. 保存PDF文件为 {taskId}.pdf
    // 3. 更新任务记录（pdf_file_path, status, completed_at）
    ```
  - [ ] 3.1.5 实现 GetPdfFilePath(taskID string) (string, error) 函数
    ```go
    // 1. 查询任务记录
    // 2. 验证status=completed
    // 3. 返回文件路径（不验证权限，MVP简化）
    ```
- [ ] 3.2 创建 `server/service/pdfexport/enter.go` 导出服务
- [ ] 3.3 在 `server/service/enter.go` 导入 pdfexport 服务
...
- [ ] 4.1 创建 `server/api/resume/pdf_export.go`
  - [ ] 4.1.1 实现 CreateExportTask handler
    ```go
    // 1. 解析请求（resume_id）
    ...
    // 5. 返回任务ID
    ```
  - [ ] 4.1.2 实现 GetExportTaskStatus handler
    ```go
    // 1. 解析路径参数（taskId）
    // 2. 调用服务层查询状态
    // 3. 返回任务状态（不验证权限，MVP简化）
    ```
  - [ ] 4.1.3 实现 DownloadExportPdf handler
    ```go
    // 1. 解析路径参数（taskId）
    // 2. 调用服务层获取文件路径
    // 3. 读取文件
    ...
    // 5. 返回文件流
    ```
- [ ] 4.2 在 `server/router/resume.go` 注册路由
  - [ ] 4.2.1 私有路由：POST /api/resume/export/create（需JWT）
  - [ ] 4.2.2 私有路由：GET /api/resume/export/status/:taskId（需JWT）
  - [ ] 4.2.3 私有路由：GET /api/resume/export/download/:taskId（需JWT）
...
## 6. Node.js子项目搭建
- [ ] 6.1 创建 `server/pdfexport-service/` 目录
- [ ] 6.2 创建 `server/pdfexport-service/package.json`
  ```json
  {
  ...
  ```
- [ ] 6.3 创建 `server/pdfexport-service/.env.example`
  ```
  PORT=3001
  ...
  LOG_LEVEL=info
  ```
- [ ] 6.4 创建 `server/pdfexport-service/.gitignore`
  ```
  node_modules/
  .env
  logs/
...
- [ ] 7.1 创建 `server/pdfexport-service/src/server.js` (单文件实现)
  ```javascript
  ...
  ```
- [ ] 7.2 实现 HTML模板渲染逻辑
  - [ ] 7.2.1 解析 resume_data JSON结构
  - [ ] 7.2.2 生成符合简历格式的HTML
  - [ ] 7.2.3 内联CSS样式（确保打印效果）
- [ ] 7.3 创建 `server/pdfexport-service/README.md` (简单说明)
  ```markdown
  # PDF Export Service
...
## 8. 前端API层 ✅ 核心 (MVP简化版)
- [ ] 8.1 创建 `web/src/api/pdfExport.ts`
  ```typescript
...
## 9. 前端编辑页集成 ✅ 核心 (MVP简化版)
- [ ] 9.1 在 `web/src/pages/editor/ResumeDetails.tsx` 添加导出选项
  ```typescript
...
## 10. 类型定义 ✅ 核心 (MVP简化版)
- [ ] 10.1 在 `web/src/types/api.ts` 添加类型
  ```typescript
